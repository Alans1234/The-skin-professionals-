...
export default function App() {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    ...
    if (saved) {
      try {
        ...
        const hasDollarOrStale = parsed.some(
          (p) =>
            (p.price && (p.price.includes("$") || !p.price.includes("Rs."))) ||
            p.name.includes("AURA Luminous") ||
            p.name.includes("Nectar"),
        );
        if (!hasDollarOrStale && parsed.length > 0) {
          ...
          return parsed.map((p) => {
            const initial = INITIAL_PRODUCTS.find(
              (ip) =>
                ip.id === p.id ||
                ip.name.toLowerCase().trim() === p.name.toLowerCase().trim(),
            );
    ...
    if (saved) {
      try {
        ...
        const hasStale = parsed.some(
          (img) =>
            img.title.includes("Golden Drop") ||
            img.subtitle.includes("copper peptide"),
        );
  ...
  const [testimonials, setTestimonials] = useState<Testimonial[]>(() => {
    ...
    if (saved) {
      try {
        ...
        if (!hasStale && parsed.length > 0) {
          return parsed.map((t) => {
            const initial = INITIAL_TESTIMONIALS.find(
              (it) =>
                it.id === t.id ||
                it.author.toLowerCase().trim() ===
  ...
  useEffect(() => {
    let title =
      "The Skin Professionals | Premium Clinical Dermatology & AI Skin Analysis";
    let desc =
      "Empower your skin with The Skin Professionals. Explore organic biological formulations, clinical diagnostics, and our advanced AI Skin Analyzer.";
    ...
    switch (activeTab) {
      case "home":
        title =
          "The Skin Professionals | Premium Clinical Dermatology & AI Skin Analysis";
        desc =
          "Empower your skin with The Skin Professionals. Explore organic biological formulations, clinical diagnostics, and our advanced AI Skin Analyzer in Kathmandu.";
        break;
      case "about":
        title = "Our Medical Board & Clinical Team | The Skin Professionals";
        desc =
          "Meet the lead formulators, dermatologists, and dermo-pathology experts behind The Skin Professionals dermo-physiological skincare solutions.";
        break;
      case "products":
        title =
          "Medical Skincare Products & Formulation Catalog | The Skin Professionals";
        desc =
          "Access clinical-grade medical skincare products, advanced hydrating matrices, and active peptide-powered formulations.";
        break;
      case "analysis":
        title =
          "AI Skin Analyzer & Custom Dermal Diagnostic | The Skin Professionals";
        desc =
          "Use our state-of-the-art server-side AI Skin Analyzer to assess hyperpigmentation, skin moisture levels, and receive professional formulation suggestions.";
        break;
      case "ingredients":
        title =
          "Botanical & Chemical Asset Transparency | The Skin Professionals";
        desc =
          "Complete absolute transparency of all physical assets inside our formulations, featuring niacinamide, salicylic acid, and hyaluronic acid.";
        break;
      case "results":
        title =
          "AI Skin Diagnostic Reports & Routines | The Skin Professionals";
        desc =
          "Access your personalized dermo-physiological dermo-analysis results, active ingredient guidance, and custom skincare routine directions.";
        break;
      case "gallery":
        title =
          "Our Treatment Gallery & Clinical Innovations | The Skin Professionals";
        desc =
          "Explore our real clinical case studies, active formulation milestones, and high-impact skincare developments in Nepal.";
        break;
      case "blog":
        title =
          "Dermatology Insights, Skin Science & Educational Blogs | The Skin Professionals";
        desc =
          "Learn best dermo-physiological skin tips, professional routines, clinical science of barrier repair, and active serum techniques from experts.";
        break;
      case "distributors":
        title =
          "Authorized Distributors & Regional Clinic Network | The Skin Professionals";
        desc =
          "Locate fully authorized skin partner locations and regional commercial representatives across Biratnagar, Nepalgunj, Chitwan, and Kathmandu.";
        break;
      case "contact":
        title =
          "Direct Contact, Assistance & Sales Office | The Skin Professionals";
        desc =
          "Connect directly with our co-founders, sales representatives, and support desk to manage accounts, dermo-clinical bookings, or bulk orders.";
        break;
      default:
        break;
    }
  ...
  return (
    <div
      className="min-h-screen bg-[#fafaf9] flex flex-col font-sans selection:bg-[#c5a880]/30 selection:text-[#052e2b]"
      id="aura-app-root"
    >
      ...
      <main className="flex-grow">
        ...
        <div id="customer-brand-routes-view">
          ...
          {activeTab === "about" && <AboutUs onNavigate={handleNavigate} />}

          {activeTab === "products" && (
            <Products products={products} onNavigate={handleNavigate} />
          )}
          ...
          {activeTab === "ingredients" && (
            <Ingredients ingredients={INITIAL_INGREDIENTS} />
          )}

          {activeTab === "results" && <Results />}

          {activeTab === "gallery" && <Gallery gallery={gallery} />}
                ...
                <div className="flex flex-col space-y-3">
                  ...
                  >
                    <a
                      ...
                      className="p-2 bg-white/5 hover:bg-white/10 text-white hover:text-brand-gold rounded-full transition-all duration-300 border border-white/5 hover:border-brand-gold/30 cursor-pointer"
