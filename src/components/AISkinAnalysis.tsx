...
export default function AISkinAnalysis({ questions, products, onSubmitRecord }: AISkinAnalysisProps) {
  ...
  return (
    <div id="ai-skin-analysis-view" className="bg-[#fafaf9] text-[#1c1c1a] min-h-screen">
      ...
      <section className="relative py-28 bg-brand-dark text-center overflow-hidden" id="analysis-header">
        ...
        <AnimatePresence mode="wait">
          ...
          {collectingInfo && (
            ...
            >
              ...
              <h2 className="font-serif text-2xl sm:text-3xl text-brand-dark mb-2 font-bold select-none">Get Your Free Skin Report</h2>
              ...
              <form onSubmit={handleInfoSubmit} className="max-w-md mx-auto text-left space-y-5" id="analysis-reg-form">
                ...
                <div className="flex flex-col">
                  <label className="font-sans text-[10px] uppercase tracking-widest text-brand-dark/60 mb-1.5 font-bold">My Full Name</label>
                  <input 
                    type="text" 
                    value={userInfo.name}
                ...
                <div className="flex flex-col">
                  <label className="font-sans text-[10px] uppercase tracking-widest text-brand-dark/60 mb-1.5 font-bold">My Email Address</label>
                  <input 
                    type="email" 
          ...
          {showResult && reportData && (
            ...
            >
              ...
              <div className="bg-white rounded-3xl border border-brand-gold/30 p-8 sm:p-12 shadow-md relative overflow-hidden text-left" id="report-sheet">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start gap-6 border-b border-stone-200/60 pb-8 mb-8" id="report-sheet-header">
                  ...
                  <div className="text-left sm:text-right" id="patient-details">
                    ...
                    <span className="font-sans text-xs text-brand-dark font-mono block mb-1">SKINPRO-{Math.floor(Math.random() * 900000 + 100000)}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start mb-10" id="report-sheet-body">
                  ...
                  <div className="md:col-span-8 space-y-6">
                    ...
                    <div className="p-4 bg-[#fafaf9] rounded-xl border border-stone-100">
                      ...
                      <p className="font-serif text-sm text-brand-dark font-bold">{reportData.concernName}</p>
                    </div>
                  </div>

                  {/* Right block metrics */}
                  <div className="md:col-span-4 bg-brand-dark/5 p-6 rounded-2xl border border-brand-dark/10 space-y-4" id="micro-telemetry">
                    <h5 className="font-serif text-xs text-brand-dark uppercase tracking-widest font-bold border-b border-brand-dark/10 pb-2">Your Core Skin Stats</h5>
                ...
                <div className="border-t border-stone-200/60 pt-8 mb-10" id="routine-blueprint">
                  <h4 className="font-serif text-lg text-brand-dark mb-4 uppercase tracking-widest font-bold">Your Personalized Daily Routine Guide</h4>
                  <div className="space-y-3">
                    {reportData.routines.map((rt, idx) => (
                      <div key={idx} className="flex items-start bg-[#fafaf9] border border-stone-250/20 p-3 rounded-xl sm:space-x-3 text-xs text-stone-600">
                        <span className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-dark/10 text-brand-dark flex items-center justify-center font-serif text-[10px] font-bold mt-0.5">{idx + 1}</span>
                    ...
                    {reportData.recomProducts.map((p) => (
                      <div key={p.id} className="bg-[#fafaf9] rounded-xl border border-stone-200/50 p-4 flex flex-col justify-between hover:border-[#c5a880]/30 transition-all">
                        <div>
                          ...
                          <h5 className="font-serif text-xs px-1 text-brand-dark font-bold leading-tight">{p.name}</h5>
                        ...
                        <div className="border-t border-stone-100/50 pt-2 mt-3 flex items-center justify-between text-[10px]">
                          <span className="font-mono text-brand-dark font-bold">Clinical grade</span>
                        </div>
                      </div>
                    ))}
                  </div>
