      ...
      <AnimatePresence mode="wait">
        {!readingBlog ? (
          ...
          >
            ...
            <section className="relative py-28 bg-brand-dark text-center overflow-hidden" id="blog-header">
                ...
                {blogs.map((b) => (
                  ...
                  >
                    ...
                    <div className="h-56 overflow-hidden relative">
                      <span className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-3 py-1 rounded-full text-[9px] font-sans tracking-widest uppercase text-brand-dark font-bold z-10">
                        {b.category}
                      </span>
                      <img 
                        src={b.image} 
                    ...
                    <div className="p-6 flex-grow flex flex-col justify-between">
                      <div>
                        ...
                        <h3 className="font-serif text-lg text-brand-dark leading-snug mb-2 group-hover:text-brand-dark/80 transition-colors">
                          {b.title}
                        </h3>
                      ...
                      <div className="flex items-center justify-between border-t border-stone-100 pt-4" id="blog-author-footer">
                        <div className="flex items-center space-x-2">
                          ...
                          />
                          <div>
                            <span className="font-serif text-xs text-brand-dark block leading-none font-semibold">{b.author.name}</span>
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1.5 text-[10px] tracking-widest uppercase text-brand-dark group-hover:text-brand-gold transition-colors font-bold cursor-pointer"
                        >
                          <span>Read</span>
          ...
          >
            ...
            <header className="mb-10 text-left">
              <span className="font-mono text-[9px] tracking-widest uppercase text-brand-dark bg-brand-gold px-3 py-1 rounded block w-max mb-4 font-bold">
                Category: {readingBlog.category} Science
              </span>
              <h1 className="font-serif text-3xl sm:text-5xl text-brand-dark tracking-tight leading-tight mb-4">
                {readingBlog.title}
              </h1>
              ...
              <div className="flex items-center space-x-3 pt-2">
                ...
                />
                <div>
                  <h4 className="font-serif text-sm text-brand-dark font-semibold">{readingBlog.author.name}</h4>
                  <p className="font-sans text-[10px] text-brand-dark/75 uppercase tracking-wider font-semibold">{readingBlog.author.role}</p>
                </div>
              </div>
            </header>
            ...
            <div className="prose max-w-none text-[#1c1c1a] font-sans text-xs sm:text-sm md:text-base leading-relaxed tracking-wide space-y-6 pt-2 pr-0 md:pr-12" id="blog-content-body">
              {readingBlog.content.split('\n\n').map((para, idx) => {
                if (para.startsWith('### ')) {
                  return <h3 className="font-serif text-xl text-brand-dark pt-4 font-bold" key={idx}>{para.replace('### ', '')}</h3>;
